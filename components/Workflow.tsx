         ...
         <div className="relative w-full grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-4">
            ...
            <div className="absolute top-1/2 left-0 w-full h-1 hidden md:block -translate-y-1/2 -z-10">
                <svg className="w-full h-20 -translate-y-[40px]" preserveAspectRatio="none">
                    <line x1="0" y1="50%" x2="100%" y2="50%" stroke="currentColor" strokeWidth="2" className="text-white/10" strokeDasharray="6 6" />
                    {/* Animated Data Flow */}
                    <circle r="4" fill="#00FFA1">
            ...
            {/* Connection Line Layer (Mobile) */}
             <div className="absolute top-0 left-8 h-full w-1 block md:hidden -z-10">
                 <div className="w-[2px] h-full bg-gradient-to-b from-transparent via-spring-neon/30 to-transparent"></div>
            </div>
            ...
            {steps.map((step, index) => (
                <div key={step.id} className="relative group">
                    ...
                    {/* Node Card */}
                    <div className="glass-cinematic p-6 rounded-2xl bg-onyx/80 border border-white/10 hover:border-spring-neon/50 transition-all duration-300 h-full flex flex-col md:items-center md:text-center text-left relative overflow-hidden group-hover:-translate-y-2 shadow-none">
                        
                        {/* Connection Point (Desktop) */}
                        <div className="absolute top-1/2 -left-2 w-4 h-4 rounded-full bg-zinc-800 border-4 border-black hidden md:block"></div>
                        <div className="absolute top-1/2 -right-2 w-4 h-4 rounded-full bg-zinc-800 border-4 border-black hidden md:block"></div>
                        
                         {/* Connection Point (Mobile) */}
                         <div className="absolute top-0 left-8 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-spring-neon md:hidden shadow-[0_0_10px_currentColor]"></div>

                        {/* Top Tag */}
                        <div className="flex justify-between items-center w-full mb-4 md:justify-center">
                             <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 text-xl">
                                {step.icon}
                             </div>
                             <span className="font-mono text-[10px] text-zinc-600 tracking-widest md:absolute md:top-4 md:right-4">
                                NODE_{step.id}
                             </span>
                        </div>

                        <h4 className="text-lg font-medium text-white mb-2 group-hover:text-spring-neon transition-colors">
                            {step.title}
                        </h4>
                        ...
                        <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-spring-neon to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    </div>
                </div>
            ))}
         </div>
