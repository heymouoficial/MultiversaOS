       ...
       <div className="max-w-6xl w-full relative z-10 flex flex-col items-center">
         ...
         <div className="text-center mb-16 md:mb-24">
             <div className="flex items-center justify-center gap-2 mb-3">
                 <span className="w-2 h-2 rounded-full bg-spring-dim dark:bg-spring-neon animate-pulse"></span>
         ...
         <div className="relative w-full grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-4">
            ...
            <div className="absolute top-1/2 left-0 w-full h-1 hidden md:block -translate-y-1/2 -z-10">
                <svg className="w-full h-20 -translate-y-[40px]" preserveAspectRatio="none">
                    ...
                    {/* Animated Data Flow */}
                    <circle r="4" fill="#00FFA1" className="hidden dark:block">
                        <animateMotion dur="3s" repeatCount="indefinite" path="M0,40 L1500,40" />
                    </circle>
                    <circle r="4" fill="#00CC81" className="block dark:hidden">
                        <animateMotion dur="3s" repeatCount="indefinite" path="M0,40 L1500,40" />
                    </circle>
                </svg>
            </div>
            ...
            {/* Connection Line Layer (Mobile) */}
             <div className="absolute top-0 left-8 h-full w-1 block md:hidden -z-10">
                 <div className="w-[2px] h-full bg-gradient-to-b from-transparent via-spring-dim/30 dark:via-spring-neon/30 to-transparent"></div>
            </div>
            ...
            {steps.map((step, index) => (
                <div key={step.id} className="relative group">
                    ...
                    {/* Node Card */}
                    <div className="glass-cinematic p-6 rounded-2xl bg-white/60 dark:bg-onyx/80 border border-black/5 dark:border-white/10 hover:border-spring-dim/50 dark:hover:border-spring-neon/50 transition-all duration-300 h-full flex flex-col md:items-center md:text-center text-left relative overflow-hidden group-hover:-translate-y-2 shadow-sm hover:shadow-xl dark:shadow-none">
                        ...
                        <div className="absolute top-1/2 -right-2 w-4 h-4 rounded-full bg-zinc-200 dark:bg-zinc-800 border-4 border-zinc-100 dark:border-black hidden md:block"></div>
                         ...
                         <div className="absolute top-0 left-8 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-spring-dim dark:bg-spring-neon md:hidden shadow-[0_0_10px_currentColor]"></div>
                        ...
                        <h4 className="text-lg font-medium text-zinc-900 dark:text-white mb-2 group-hover:text-spring-dim dark:group-hover:text-spring-neon transition-colors">
                            {step.title}
                        </h4>
                        ...
                        <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-spring-dim dark:via-spring-neon to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    </div>
                </div>
            ))}
         </div>
