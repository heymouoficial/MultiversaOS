...
const TechStack: React.FC<TechStackProps> = ({ text }) => {
  const stack = [
    { 
        name: text.desc1, 
        desc: text.sub1, // "SIEMPRE ACTIVO" or "ALWAYS ON"
        icon: (
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center items-center px-4 relative py-32 md:py-0">
       {/* Transparent Background to respect Onyx Global */}
      ...
      <div className="max-w-5xl mx-auto z-10 w-full flex flex-col items-center">
         ...
         <div className="mb-24 text-center w-full max-w-2xl border-b border-white/5 pb-10">
            <div className="flex items-center justify-center gap-2 text-spring-neon font-mono text-xs tracking-widest uppercase mb-4">
                <span className="w-2 h-2 bg-spring-neon rounded-full animate-pulse"></span>
                {text.tag}
            </div>
            <h2 className="text-4xl md:text-6xl font-light text-white mb-4 tracking-tighter">{text.title}</h2>
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
            {stack.map((item, i) => (
                <div 
                    key={i} 
                    className="glass-cinematic p-8 rounded-2xl flex flex-col items-center text-center group bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-white/10 transition-all duration-500 hover:-translate-y-2 min-h-[220px] justify-center"
                    style={{ transitionDelay: `${i * 100}ms` }}
                >
                    <div className="mb-6 transform transition-transform duration-500 ease-out scale-100 group-hover:scale-110 group-hover:text-spring-neon">
                        {item.icon}
                    </div>
                    <h4 className="text-white font-medium text-lg mb-2 tracking-wide group-hover:text-spring-neon transition-colors">{item.name}</h4>
