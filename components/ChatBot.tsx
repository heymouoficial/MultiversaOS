                    ...
                    {stage === 'ticket_ready' && reservationData.ticketId && (
                         <div className="animate-reveal mx-auto max-w-[90%] bg-zinc-900 border border-spring-neon/30 rounded-xl p-0 overflow-hidden shadow-[0_0_30px_rgba(0,255,161,0.1)] mb-4 mt-4">
                             <div className="bg-spring-neon p-3 flex justify-between items-center">
                                 <span className="text-black font-bold font-mono text-xs tracking-widest">{text.ticket.title}</span>
                             ...
                             <div className="p-5 space-y-3 relative">
                                 ...
                                 <div className="relative z-10">
                                     <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{text.ticket.plan}</p>
                                     <p className="text-white font-medium text-lg">{reservationData.plan}</p>
                                 </div>
                                 <div className="relative z-10">
                                     <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{text.ticket.user}</p>
                                     <p className="text-white font-medium">{userName || 'Guest'}</p>
                                 </div>
                                 <div className="relative z-10 flex justify-between">
                                     <div>
                                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{text.ticket.location}</p>
                                        <p className="text-zinc-300 text-sm">{reservationData.location}</p>
                                     </div>
                                     <div className="text-right">
                                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider">{text.ticket.status}</p>
                                        <span className="text-spring-neon text-xs font-mono border border-spring-neon/30 px-2 py-0.5 rounded bg-spring-neon/10 animate-pulse">{text.ticket.statusVal}</span>
                                     </div>
                                 </div>
                             </div>
                             
                             <div className="p-4 bg-[#050505] border-t border-white/5 text-center">
                                 <p className="text-zinc-500 text-xs mb-3">{text.ticket.footer}</p>
                                 <a 
